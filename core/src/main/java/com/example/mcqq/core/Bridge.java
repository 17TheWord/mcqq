...
public final class Bridge {
    ...
    public List<String> runCommand(String command) {
        Optional<Rcon.Endpoint> endpoint = Rcon.endpointFrom(serverProperties());
        if (endpoint.isEmpty()) {
            return List.of("服务端没开 RCON，命令执行用不了 —— 在 server.properties 里设 enable-rcon=true "
                    + "与 rcon.password=<密码>，重启服务器后生效");
        }
        try {
            List<String> lines = Rcon.run(endpoint.get(), command);
            return lines.isEmpty() ? List.of("（这条命令没有回显）") : lines;
        } catch (IOException e) {
            // 最常见的两种失败：服务端没在跑 RCON，或者这条命令**本身就是从 RCON 发过来的** ——
    ...
    private Path serverProperties() {
        ...
        Log.debug("configDir=" + configDir + " → 服务端目录=" + serverDir);

...
运维会看到的是：装好、填好密钥、机器人连上了、群里说话了 —— 然后**什么都没有**。
...
3. **绑上**：`/qq bind` 绑**最近说话的那个群**；`/qq bind <openid 前几位>` 指定一个（前缀唯一才认，歧义就拒绝）。
   它把群写进 `config.yml`（先备份 `.bak`）然后自动重载 —— 全程不用打开文件。

**安全性质**：`/qq bind` 只接受**机器人真的收到过消息**的群，所以打错字不可能把聊天栏接到陌生的群去。

**顺带**：`bot X bridges no group` 那句提示改成同时说两条路（填文件 / 敲 `/qq bind`）—— 运维看的正是这句。
...
**问题**：原来只支持环境变量（`secret-env:`）—— 而运维手里有什么、能改什么，不该由我们来假定。
而且"只支持环境变量"也**没有换来什么安全**：能改这个文件的人本来就能改启动方式，只是白设了一道门槛。
...
所以只进 `/qq status`、**不进日志**。对一个刚粘完配置、什么都没发生、只会看日志的运维来说，这是最糟的
...
[mcqq] config: bot no-secret-anywhere: 没有可用的 AppSecret —— 在 config.yml 里写
       secret: <AppSecret>，或设环境变量 QQ_BOT_SECRET；已跳过                ← 这条现在进日志了
```
...
**为什么两种都留着**："别让密钥跟着 config.yml 一起被备份 / 粘进 issue"这个理由仍然成立，
所以 `secret-env` 保留；但它是另一个选项，不是唯一的路。

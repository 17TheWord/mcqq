...
## 绑定群聊：不用再手抄 openid（2026-09-22，最新，用户提出）

**问题**：配置要填 `group-openid`，而拿到它的办法是"在群里 @ 一次机器人然后看日志" —— 可机器人得先连上、
配置里还得先有群；更要命的是**没绑定的群的消息只走 `Log.debug`**，而 debug 默认关着。
面板服主会看到的是：装好、填好密钥、机器人连上了、群里说话了 —— 然后**什么都没有**。

**方案**（三层，从"一定能发现"到"不用碰文件"）：

1. **记下来**：没绑定的群进 `UnboundGroups`（住在 `Bridge` 里，所以活过 `/qq reload`；上限 16 条）。
   首次见到打**一条 WARN**，只一次而不是每条消息一次 —— 否则一个热闹的群会刷爆日志：
   ```
   [mcqq] config: 收到群 <openid> 的消息，但它没绑定 —— 在控制台敲 /qq bind 就能绑上
          （或把 <openid> 填进 config.yml 的 group-openid）
   ```
2. **列出来**：`/qq status` 多一段「收到过消息但没绑定的群」，带 bot id 与时间。
3. **绑上**：`/qq bind` 绑**最近说话的那个群**；`/qq bind <openid 前几位>` 指定一个（前缀唯一才认，歧义就拒绝）。
   它把群写进 `config.yml`（先备份 `.bak`）然后自动重载 —— 面板服主全程不用打开文件。

**安全性质**：`/qq bind` 只接受**机器人真的收到过消息**的群，所以打错字不可能把聊天栏接到陌生的群去。

**顺带**：`bot X bridges no group` 那句提示改成同时说两条路（填文件 / 敲 `/qq bind`）—— 面板服主看的正是这句。

**为什么敢让插件改配置文件**：这个项目的设计早就把"配置文件可被重写"当成了前提 —— 注释住在
`config.example.yml`，改动前留 `.bak`，`config.yml` 只是机器管的那份。所以 `bindGroup` 走的是同一个契约。

**验证**：

* **单测**（新增 9 个，core **51 → 60**）：`UnboundGroupsTest` 5 个（首次才报、最新怎么算、前缀唯一/歧义、
  绑过就忘、**封顶**）；`BindFlowTest` 2 个把整条链跑完（`QqToMc` 收到群消息 → 登记 → `/qq bind` → 写文件
  → 重载 → 状态里出现该群）；`BridgeConfigTest` 2 个（写进对的 bot、留备份、重复绑被拒）。
  整条链**不碰网络**：配置里写一个永不存在环境变量名，`register()` 直接跳过，所以没有 HTTP 调用。
  ⚠️ 封顶那个测试**当场抓出我漏了 `removeEldestEntry`** —— 我写的时候以为 `LinkedHashMap` 有上限。
* **真机**（Paper 1.20.1，RCON）：`/qq help` 列出 `bind`；`/qq bind` 在空状态下给出正确指引；
  `/qq status` 的"没绑任何群"提示提到了新路；日志里能看到那条 WARN 与 `bridging bot main with 0 group(s)`。
* ⚠️ **没验到的一步**：真实 QQ 群消息 → `/qq bind` 成功。那需要一个真机器人（`EventBus.dispatch` 是公开的，
  但服务器进程里没有注入入口）。这一步由 `BindFlowTest` 用真实的 `QQMessageEvent` 覆盖到"写文件 + 重载"为止；
  剩下的只是"SDK 会不会真的投递这个事件"，而那是既有代码，这次没动。

## AppSecret 可以直接写在 config.yml 里（2026-09-22，用户提出）

**问题**：原来只支持环境变量（`secret-env:`），而**面板服不让服主设环境变量** —— 等于把大多数会用这个 mod 的人
挡在门外。用户还指出：面板服上服务商本来就能看到进程环境，所以"只支持环境变量"在那里**并没有换来任何安全**，
只是白设了一道门槛。

**改法**：bot 支持 `secret: <AppSecret>`（**主路径**），`secret-env:` 保留给自建服（想把密钥留在文件外的那种）。
两个都填**以文件里的为准**，并在启动时说明一声 —— 静默挑一个就是"为什么还在用旧密钥"的开端。
模板里的 `REPLACE_ME` 视为"没填"（否则会把占位符当密钥发给平台，换来一次登录失败而不是一句提示）。

**顺手修掉一个更严重的旧问题**：`register()` 里加的"跳过这个 bot"是在**打印问题列表之后**才加进去的，
所以只进 `/qq status`、**不进日志**。对一个刚粘完配置、什么都没发生、只会看日志的面板服主来说，这是最糟的
失败模式 —— 一句解释都没有。现在 `start()` 会把 `register()` 新增的问题也打出来。

**验证**（一次真机跑同时验两条路径）：`run-1.20.1` 的 config.yml 放两个 bot ——

```
[mcqq] bridging bot from-file with 1 group(s)                     ← 文件里的 secret 被读到了
[mcqq] config: bot no-secret-anywhere: 没有可用的 AppSecret —— 在 config.yml 里写
       secret: <AppSecret>（面板服推荐），或设环境变量 QQ_BOT_SECRET（自建服）；已跳过   ← 这条现在进日志了
```

`from-file` 用的是假密钥，所以后面能看到网关重连重试（说明它**真的拿这个密钥去连了**）；
`/qq status` 里两个 bot 的状态也各自对（`[未连接]` / `[未注册]`）。

**测试**：core **47 → 51** 个用例（新增 4 个：文件里的 secret 被读到、`REPLACE_ME` 视为没有、
两个都填会被报告、省略 `secret-env` 仍走老默认 `QQ_BOT_SECRET`）。

**为什么没把 `secret-env` 删掉**："别让密钥跟着 config.yml 一起被备份 / 粘进 issue"对**自建服**仍然成立。
但那条不构成让面板服用户为难的理由，所以它降级成备选，不再是唯一的路。

## Bukkit 一族改成"编译对最老目标"：一个 jar 覆盖 1.20.1 → 26.x（2026-09-22）
